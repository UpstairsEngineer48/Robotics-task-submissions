...
class Task3C(Node):
    ...
    def control_loop(self):
        ...
        diffx = self.goal_x - self.thief_pose.x
        diffy = self.goal_y - self.thief_pose.y
        ...
        diffx = self.thief_pose.x - self.police_pose.x
        diffy = self.thief_pose.y - self.police_pose.y
        ...
        if distance < 0.5:
            ...
            self.police_pub.publish(stop)
            self.thief_pub.publish(stop)
        ...
        police_cmd.linear.x = diffx
        police_cmd.linear.y = diffy
